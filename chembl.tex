...
\setbeamercolor{byell}{bg=RoyalBlue}
...
\setbeamertemplate{footline}
{
  ...
  \end{beamercolorbox}%
  \begin{beamercolorbox}[wd=.4\paperwidth,ht=2.25ex,dp=1ex,center]{byell}%
    \usebeamerfont{date in head/foot}%
	\raisebox{-0.25em}{\includegraphics[width=0.25in]{images/Chembl}}%\hspace*{-1em}%
  \end{beamercolorbox}}%
}
...
\date{\today}
  ...
  \begin{document}
	...
	\frame[plain]
	{
		\frametitle{The problem of haplotype frequency estimation -- Bachelor thesis}
		...
		$\bullet$~In general, it could be impossible to infer haplotypes from genotype data.
	...
	\frame[plain]
	{
		\frametitle{Idea of short overlapping window}

		\begin{block}{Problem}
			Every algorithm employing full space search would operate with $O(c^{n})$ complexity.
		...
		\begin{figure}
			\begin{columns}%
				\begin{column}{0.3\textwidth}%
					\includegraphics[height=0.25\textheight]{images/window}
				\end{column}%
				\begin{column}{0.53\textwidth}%
					\caption{\small{What are the error and execution time as a function of \textbf{width} and \textbf{overlap} parameters?}}
				\end{column}%
			\end{columns}
		\end{figure}
	}
	...
	\frame[plain]
	{
		...
		\begin{block}{Functional genomics as a major field in applied bioinformatics}
			\begin{itemize}
				...
				\item This task cannot be done without extensive functional annotation of the datasets.
				...
				\item Standarized functional annotation is essential.
			\end{itemize}
		\end{block}
				
		\begin{block}{The goal}
			Provide biologists with useful information to take into account when addressing the task of functionally characterizing their sequence data.
		\end{block}
	}
	...
	\frame
	{
		...
		\begin{itemize}
			...
			\item Data fusion and inference.
		\end{itemize}
	...
	\frame
	{
		\frametitle{Site Status Board -- an application monitoring the behaviour of all the centers of a particular VO}
	...
	\frame
	{
		...
		\begin{block}{Benefits for the dashboard framework:}
			...
			\begin{itemize}
				...
				\item Authentication mechanisms implemented in framework.
	...
	\frame
	{
		...
		\begin{itemize}
			\item Dashboard for Google Earth.
			\item SiteView.
		\end{itemize}
	}
	...
	\frame
	{
		...
		\begin{itemize}
			...
			\item Many interesting lectures (including those by Richard Stallman and James Watson).
			\item French language course.
			\item Working in multinational environment.
	...
	\frame
	{
		...
		Development of business applications intended for use by corporate clients:
